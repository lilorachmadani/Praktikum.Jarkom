...
TCP (Transmission Control Protocol) adalah protokol jaringan yang memastikan data dikirim dan diterima dengan benar serta berurutan. TCP bekerja dengan membangun koneksi, memeriksa keutuhan data, dan mengirim ulang paket yang hilang atau rusak. Karena keandalannya, TCP banyak digunakan pada layanan seperti web, email, dan transfer file.

## 6.2 Menangkap Tansfer TCP dalam Jumlah Besar dari Komputer Pribadi ke Remote Server
Analisis transfer TCP bertujuan untuk mengamati proses pengiriman data dari komputer ke server melalui jaringan. Pada praktikum ini, pengamatan dilakukan menggunakan Wireshark yang mampu menangkap dan menampilkan informasi paket TCP, seperti alamat IP, nomor port, dan ukuran paket. Hasil analisis digunakan untuk mengevaluasi kinerja jaringan, mengidentifikasi masalah komunikasi, serta mendeteksi aktivitas jaringan yang tidak normal.
